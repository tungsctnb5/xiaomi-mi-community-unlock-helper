...
class LoginWindow(QMainWindow):
    ...
    def __init__(self, profile_dir:Path, *, offline=False):
        ...
        if offline:
            self.view.setHtml("<html><body><p id='probe'>Offline renderer ready</p></body></html>")
        else:
        ...
        if not self._captured and bytes(cookie.name()).decode(errors="ignore")=="new_bbs_serviceToken":
            ...
            if value:
                ...
                self.view.setHtml("""<html><body style='background:#171717;color:#eee;font:20px -apple-system;padding:40px'>
                  ...
                  <p>Your Xiaomi Community session was captured. Check the main window for session and storage status.</p>
...
def clear_browser_session(profile_dir: Path, parent=None):
    ...
    if sys.platform.startswith("linux"):
        # The login window owns an off-the-record profile; deleting it clears it.
        return None
    else:
